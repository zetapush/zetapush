...
const getErrorsFromAssociatedLogsStr = (step) => {
  return step.associatedLogs
          ...
          .map(e => e.message.replace(/^(.*)$/g, "          $1")).join('\n')
}
...
const writeLogs = (config, logs, steps) => {
  ...
  fs.writeFileSync(logFile, `Context:
...
const getLogsStr = (logs, steps) => {
  return logs.map(log => `${fixedSize(getStepName(log.step, steps), 50)} ${fixedSize(log.category.name, 20)} | ${formatDate(log.timestamp)} [${log.level.name}] ${log.category.context || ""} ${log.message}`).join("\n")
}
