...
class NetworkIssueAnalyzer extends ErrorAnalyzer {
  ...
  isBehindProxy(err) {
    ...
    return code=='ENETUNREACH'
          || (code=='ECONNREFUSED' && !err.request.url.contains(err.failure.address))   // if address different from URL, request goes through a proxy (TODO: is always working?)
  }
  ...
  async getError(err) {
    ...
    try {
      ...
      // TODO: return default resolution help
      trace(e)
    }
  }
}
