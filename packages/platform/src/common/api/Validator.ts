...
export class ValidationAccountCreationManager implements ValidationManager {
  validate(object: Object, validationSchema: ValidationSchema) {
    validate(validationSchema.name, object).then((errors) => {
      if (errors.length > 0) {
        ...
        throw new AccountCreationValidationError(
          'ACCOUNT_CREATION_VALIDATION_ERROR',
