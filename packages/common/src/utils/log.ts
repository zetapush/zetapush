...
export const debugObject = (what: string, ...objects: any[]) => {
  ...
  for (let obj of objects) {
    for (let name in obj) {
      let file = getZetaFilePath('zeta-debug', `${Date.now()}-${what}-${name}.json`);
      try {
        fs.writeFileSync(file, stringify(obj[name], null, 2));
        console.debug(chalk`{gray [TRACE] ${what}-${name} written in ${file}}`);
      } catch (e) {
        try {
          let fallbackFile = getZetaFilePath('zeta-debug', `${Date.now()}-${what}-${name}.string`);
          fs.writeFileSync(fallbackFile, Object.prototype.toString.call(obj[name]));
          console.debug(chalk`{gray [TRACE] ${what}-${name} written in ${fallbackFile}}`);
        } catch (err) {
