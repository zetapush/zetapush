...
import { TimestampBasedUuidGenerator, ZetaPushContext, ConfigurationProperties } from '../../common/core';
import { AccountStatus, Account } from '../api';
import { DefaultUsernamePasswordAccountDetails } from '../core';
import { StandardAccountStatus } from '../core/account';
import { Variables } from '../../common/api';
  ...
  configureAccountRegistration(registrationConfigurer: RegistrationConfigurer) {
    registrationConfigurer
      .account()
        ...
        .fields()
          .scan()
            .annotations(DefaultUsernamePasswordAccountDetails)
            .and()
          .and()
        .and()
      // .fields()
      ...
      .confirmation()
        .email()
          ...
          .htmlTemplate(/*new MustacheTemplateProvider('templates/email/confirm.html')*/)
            .template(({account}: {account: Account}) => `Hello ${account.userProfile.username}, Please confirm your account`)
            .inlineCss(/*new CssInliner('styles')*/)
            .inlineImages(/*new Base64ImageInliner('images')*/)
            .and()
